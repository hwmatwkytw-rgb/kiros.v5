...
module.exports.config = {
    ...
    description: "توليد صور فائقة الدقة مع ترجمة تلقائية (توليد [الوصف])",
    commandCategory: "الذكاء الصناعي",
    usages: "توليد [وصف الصورة]",
    cooldowns: 20
};
...
module.exports.run = async function ({ api, event, args }) {
    ...
    if (!prompt) return api.sendMessage("┌──── • ⚠️ • ────┐\n يرجى كتابة وصف بعد كلمة توليد\n└──────────────┘", threadID, messageID);

    const magic = new MagicAi();
    api.setMessageReaction("⌛", messageID, () => {}, true);
    
    const waitMsg = await new Promise(res => api.sendMessage("┌─── [ ⚙︎ GENERATING ] ───┐\n│ جاري ابتكار لوحتك الفنية... │\n└──────────────────┘", threadID, (err, info) => res(info)));

    try {
        // الترجمة التلقائية للإنجليزية
        ...
        const cachePath = path.join(__dirname, 'cache', `gen_${Date.now()}.jpg`);
